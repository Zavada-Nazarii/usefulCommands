...
```
```bash
socat -T300 TCP4-LISTEN:4444,reuseaddr,range=192.168.141.10/32,max-children=1 TCP4:192.168.141.129:4444
```
🔍 Пояснення:

- без fork – означає, що лише одне з’єднання обслуговується;
...
- max-children=1 — лише одне з’єднання.
